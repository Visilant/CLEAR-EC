...
@torch.no_grad()
def run_batch(model, frames_u8, device, threshold, nms_px, amp=True):
    """frames_u8: (B,H,W) uint8 numpy -> list of per-frame dicts."""
...
def consolidate(det_dir, n, out_path, log):
    xs, ys, ss, offs = [], [], [], [0]
...
def main():
    ...
    if len(glob.glob(os.path.join(det_dir, "*.npz"))) >= n:
        consolidate(det_dir, n, os.path.join(args.out, "detections.npz"), log)
