...
> Paths in section 8 were updated on 2026-09-13 after the repository consolidation (one branch, trainer split into modules, `experiments/` runner, ledger in `docs/EXPERIMENTS.md`).
...
- Target: per image, CD (cells/mm2), CV (area coefficient of variation), HEX (fraction six-sided). Score is
  the equal-weight mean of the three MAPEs. 100 hidden test images in Phase I, top 5 advance (submissions unlimited, container uploads limited; corrected 2026-09-14, see docs/HANDOFF_2026-09-14.md);
...
| Noise-floor scripts | `code/scripts/noise_floor/{overlay_floor.py,oof_residuals.py,compare_fold.py}` (repo-relative defaults) |
| Tests | `code/tests/` (unittest; `test_noise_floor.py`, `test_parity.py`, `test_folds.py`, `test_spec.py`, ...) |
| Trainer | `code/src/training/{config,data,models,losses,loop,folds,predict,train}.py`; `code/scripts/run_training.py --config_json`; campaigns via `code/experiments/run.py` and `code/experiments/specs/*.yaml` |
| Scorer with flip TTA and index subsets | `code/src/training/predict.py` (`predict_tta`, `VIEWS`) and `experiments/score.py`; parity-tested against `results/night_20260912/golden/` |
| Fold definition | `src/training/folds.py::_slide_group_folds(cache_dir, 5, seed=42)` (deterministic; frozen by `tests/test_folds.py`) |
...
| Overlay click sets and Voronoi readout | `results/sam_spike_20260912/overlays.json`; `code/phase2/detector/{reference_readout.py,readout.py}` |
...
- (Resolved 2026-09-13: all work is committed on `main`; the worktrees were merged and removed.)
